...
struct dqrl : public algorithm {
	std::vector<double> V1 = std::vector<double>(env.Ns,0.0);
	std::vector<double> V2 = std::vector<double>(env.Ns,0.0);
	matrix p;

	dqrl(Environment env, Parameters par_) :
		algorithm{env,par_},
	...
	void episode() override {
		for(state_t s = 0; s < env.Ns; s++){
			...
			std::size_t L = std::max(0,std::min(int(par.k*(r+(V1[sp] + V2[sp])/2)), int(M_PI/(4*the) - 0.5 + 1e-6)));
			...
			if(random_choice(0,1) == 0)
				V1[s] += par.alpha*(r + par.gamma*env.grid[sp]*V2[sp] - V1[s]);
			else
				V2[s] += par.alpha*(r + par.gamma*env.grid[sp]*V1[sp] - V2[s]);
		}
	}
	...
	test_t test(std::size_t maxi) override {
		...
		t.double_output = (V1 + V2) / 2;
		return t;
	}

	std::ostream& print(std::ostream& o) override {
		return o << "V: " << std::endl << (V1+V2)/2 << std::endl << "p: " << std::endl << p;
	}
};

int main(int argc, char** argv){
	return main_func<dqrl>(argc,argv);
}
